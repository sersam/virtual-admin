...
export function createTextPdfBlob(input: TextPdfInput): Blob {
...
export function downloadTextPdf(input: TextPdfInput & { readonly filename: string }): void {
  const url = URL.createObjectURL(createTextPdfBlob(input));
...
function normalizePdfContent(input: TextPdfInput): string {
